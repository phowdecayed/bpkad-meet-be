...
-   **Payload Parameters:**
    ...
    | `notulen` | string | nullable | Manual meeting minutes (notulen). |
    | `status` | string | nullable | Initial status (scheduled, started, finished, canceled). Default: scheduled. |
...
-   **Payload Parameters:** (All are optional)
    ...
    | `notulen` | string | nullable | Manual meeting minutes. |
    | `status` | string | in:scheduled,started,finished,canceled | Update meeting status. |
...
-   **Method:** `GET`
-   **Endpoint:** `/api/meetings/{id}/attendances`
-   **Description:** Retrieves the list of known attendees. **Requires `view meetings` permission.**
-   **Headers:** `Authorization: Bearer <token>`
-   **Success Response (200):** A collection of attendance records.

### 11. Export Attendance (CSV)

-   **Method:** `GET`
-   **Endpoint:** `/api/meetings/{id}/attendances/export`
-   **Description:** Downloads the attendance list as a CSV file. **Requires `view meetings` permission.**
-   **Headers:** `Authorization: Bearer <token>`
-   **Success Response (200):** A file download stream (`Content-Type: text/csv`).

### 12. Upload Material

-   **Method:** `POST`
-   **Endpoint:** `/api/meetings/{id}/materials`
-   **Description:** Uploads a file attachment (PDF, PPT, Image) to the meeting. **Requires `edit meetings` permission.**
-   **Headers:** `Authorization: Bearer <token>`, `Content-Type: multipart/form-data`
-   **Payload:**
    | Parameter | Type | Validation | Description |
    |---|---|---|---|
    | `file` | file | required, max:10MB | The file to upload (pdf,doc,docx,ppt,pptx,jpg,png,jpeg). |
-   **Success Response (201):** The created material object.

### 13. Delete Material

-   **Method:** `DELETE`
-   **Endpoint:** `/api/materials/{materialId}`
-   **Description:** Deletes a meeting material. **Requires `edit meetings` permission.**
-   **Headers:** `Authorization: Bearer <token>`
